...
function doMyJob(){
    ...
    if(dataLoaded){
        ...
        bdd.query('SELECT assignations.id_mytime, journee, date_start,jour,chantiers.id_mytime AS chantier_id, resources.id_mytime AS resource_id' +
            ...
            ' AND resources.Actif=1 AND resources.type="SALARIE" AND resources.id_mytime IS NOT NULL AND assignations.id_mytime IS NULL ' + //AND resources.matricule_resource = "000448"
                ...
                if (error) {
                    ...
                    for (let ass of chantiersMySql) {
                        ...
                        if (x == 1) {
                            ...
                            var start_time = ((ass.journee == 0 || ass.journee == 1) ? '07:00:00' : '12:45:00');
                            var end_time = ((ass.journee == 0 || ass.journee == 2) && ass.jour != 4 ? '15:45:00' :
                    ...
                    const formData = {
                        data: JSON.stringify(SYNC_DATA.data)
                    }
                    ...
                    //console.log(util.inspect(formData.data, false, null, true /* enable colors */))
                    //process.stdout.write(options.formData.data)
                            ...
                            if(resultPostParse != null && typeof resultPostParse.schedules != 'undefined' && resultPostParse.schedules != null){
                                ...
                                for(let sc of resultPostParse.schedules){
                                    ...
                                    && sc.registration_number != null && typeof sc.registration_number != 'undefined'){
                                        ...
                                        upd_request += 'UPDATE assignations SET id_mytime = ? WHERE id_assignation= ?; INSERT INTO mytime_planning (id_mytime) VALUES (?); ';
...
function createTime(list , journee, date, resource_id){
    if(journee ==0 ||journee == 1) {
        ...
        for(let el of list) {
            let obj = el;
            if (obj.employee == resource_id && obj.start_date== date && obj.journee == journee) {
