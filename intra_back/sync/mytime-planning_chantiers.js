            ...
            bdd.query('SELECT * FROM mytime_planning WHERE mytime_planning.id_mytime NOT IN (SELECT DISTINCT(assignations.id_mytime) FROM assignations, resources WHERE resources.Actif = 1 AND resources.id_mytime IS NOT NULL AND resources.matricule_resource = assignations.matricule_resource AND assignations.id_mytime IS NOT NULL)',
                function (error, toRemove, fields) {
                    if (error) {
                        ...
                        bdd.query('SELECT * FROM resources WHERE resources.Actif=1 AND resources.type="SALARIE" ' +
                            ...
                            'AND resources.id_mytime IS NOT NULL ',
                        function(error, ressources, fields){
                            ...
                            ressources.forEach((ass) => {
                                ...
                                var usr = [];
                                usr.id = ass.id_mytime;
                                usr.chantiers = [];

                                let params_assignations = {
                                    'status': 'ACTIF',
                                    'employee_id': ass.id_mytime,
                                    'assigned': 1
                                };
                                ...
                                //https://api-preprod.mytime.fr/v2/companies/?&status=ACTIF&employee_id=R7cx57h1ty49ZSodG8ab&assigned=1
                                request.get(options_assignations, function (error, assignation, params_assignations) {
                                    requests_count++;
                                    var predicate = (e) => e.id_mytime == ass.id_mytime;
                                    DATAS_job[DATAS_job.findIndex(predicate)]._done = true;

                                    if (error) {
                                        log("Erreur synchronisation MyTime _ ASSIGNATIONS : " + error, 'Planning Chantiers', null)
                                    } else {
                                        var answer = JSON.parse(assignation.body);
                                        answer.forEach((el) =>{
                                            usr.chantiers.push(el);
                                        });
                        ...
                        timer_job = setInterval(doMyJob, 5000);
                    }
                });
                        ...
                        setTimeout(
                            function() {
                                ...
                                if (x == 1) {
                                    ...
                                    body = body + '&start_time=' + ((ass.journee == 0 || ass.journee == 1) ? '07:00:00' : '12:45:00');
                                    body = body + '&end_time=' +
                                        ((ass.journee == 0 || ass.journee == 2) && ass.jour != 4 ? '15:45:00' :
                                    ...
                                    body = querystring.stringify(body)
                                    body = body + '&start_time=' + res[0];
                                    body = body + '&end_time=' + res[1];
                                }
                                //remplissage du planning chantier

                                assSendList.push(body);
                                //console.log(body)
                                var options = {
                                    url: 'https://apiold.mytime.fr/v2/schedules/',
                                    headers: {
                                        'Authorization': TOKEN_TYPE + ' ' + TOKEN,
                                        'Content-Type': 'application/x-www-form-urlencoded'
                                    },
                                    ...
                                    request.post(options, function (error, resultPost, body) {
                                        ...
                                        if (error) {
                                            ...
                                            try{
                                                ...
                                                console.log("Receivend invalid Assignation : " + ass);
                                                console.log(ass);
                                            }

                                            if(resultPostParse != null && typeof resultPostParse.id != 'undefined' && resultPostParse.id != null){
                                ...
                                var pred = (e) => e.id == ass.resource_id;
                                var ind = RES_AFF.findIndex(pred);
                                var idx = -1;

                                //console.log(RES_AFF)

                                if(ind !== -1){
                                    var predicate = (e) => e.id == ass.chantier_id;
                                    idx = RES_AFF[ind].chantiers.findIndex(predicate);

                                    //console.log(RES_AFF[ind].chantiers)
                                } else {
                                    console.log("IND === -1")
                                }
                                ...
                                /*if (idx === -1) {
                                    ...
                                    request.post(options, function (error, assignation, body2) {
                                        requests_count++;
                                        if (error) {
                                            log("Erreur synchronisation MyTime _ Assignation Chantier : " + error, 'Planning Chantiers', null)
                                        } else {
                                        }
                                    });
                                }*/
